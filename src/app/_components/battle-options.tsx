...
export default function BattleOptions({ onOptionSelected, onOptionFocus, onOptionBlur, disabled }: BattleOptionsProps) {
  ...
  const baseTw = "w-full md:max-h-[3rem] text-center px-3 py-2 mb-1 border border-1 border-sky-100 focus:outline-none focus:bg-gray-700 focus:ring-2 focus:ring-blue-500";
  const xTw = disabled ? "cursor-not-allowed bg-gray-700 text-gray-400" : "hover:bg-gray-700 cursor-pointer";
  const tw = `${baseTw} ${xTw}`;

  const divGrid = options.length > 1 ? "md:grid md:grid-cols-2 md:grid-rows-2" : "md:grid md:grid-cols-1 md:grid-rows-1";

  return (
    <div className={`border border-2 border-slate-200 text-white p-4 w-full font-mono select-none flex flex-col ${divGrid} gap-3`}>
